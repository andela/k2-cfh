angular.module('mean.system')
  ...
  .factory('Custom', ['$window', '$location', function ($window, $location) {
    return {
      checkIfPresenceAllowed() {
        const avatar = $window.localStorage.getItem('avatar');
        const token = $window.localStorage.getItem('token');
        if (typeof avatar === 'string' && typeof token === 'string') {
          return $location.path('/app');
        }
        if (typeof avatar === 'string' || typeof token !== 'string') return $location.path('/');
      }
    };
  }])
